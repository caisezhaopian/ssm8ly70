	...
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="退租日期" prop="tuizuriqi">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.tuizuriqi" 
						type="date"
						:readonly="ro.tuizuriqi"
						placeholder="退租日期"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.tuizuriqi" label="退租日期" prop="tuizuriqi">
					<el-input v-model="ruleForm.tuizuriqi" placeholder="退租日期" readonly></el-input>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.tuizuriqi = this.getCurDate()
	},
	methods: {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yewutuizu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yewutuizuCrossAddOrUpdateFlag = false;
						 this.parent.search();
